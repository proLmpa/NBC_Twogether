...
public class WorkspaceMemberController {
    ...
    public ResponseEntity<ApiResponseDto> createWorkCollabo(
        @AuthenticationPrincipal UserDetailsImpl userDetails, @PathVariable Long workspaceId,
        @PathVariable Long boardId, @PathVariable Long userId) {
        workspaceMemberService.createWorkCollabo(userDetails.getUser(), workspaceId, boardId,
            userId);
        return ResponseEntity.ok().body(new ApiResponseDto(HttpStatus.OK.value(),
            "Workspace Collaborator Member Creation Success!"));
    }

    @Operation(summary = "워크스페이스 협업 멤버 삭제")
    @DeleteMapping("/workspaces/{id}/invite")
    public ResponseEntity<ApiResponseDto> deleteWorkCollabo(
        @AuthenticationPrincipal UserDetailsImpl userDetails, @PathVariable Long workspaceId,
        @PathVariable Long boardId, @PathVariable Long userId) {
        workspaceMemberService.outWorkCollabo(userDetails.getUser(), workspaceId, boardId, userId);
        return ResponseEntity.ok().body(new ApiResponseDto(HttpStatus.OK.value(),
            "Workspace Collaborator Member Delete Success!"));
    }
}